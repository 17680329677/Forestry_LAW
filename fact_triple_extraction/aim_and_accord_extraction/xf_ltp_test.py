...
def content_nlp(content):
    words_list = word_segment(content)      # 分词结果
    postags_list = word_postag(content)     # 词性标注结果
    dp_info = dependency_parse(content)     # 依存句法分析
    srl_info = semantic_role_labeller(content)      # 语义角色标注分析
    sdgp_info = semantic_dependency_parse(content)      # 语义依存分析
    dp_tags, sdp_tags = get_tags()      # 获取标签（依存句法分析、语义依存分析）对应字典
    ...
    for index in range(len(dp_info)):
        ...
        print(parent_word, '----(', reletion_name, ')----', child_word)
    ...
    for sdp_index in range(len(sdgp_info)):
        ...
        if sdgp_info[sdp_index]['relate'] in sdp_tags:
            ...
            main_relate = '' + str(sdgp_info[sdp_index]['relate'])[1:]
            semantic_dp_name = sdp_tags['Poss'] + '--嵌套角色'
        else:
