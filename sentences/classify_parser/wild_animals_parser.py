# -*- coding : utf-8 -*-
# coding: utf-8
# 野生动物条例相关法律法规的解析---并存入mysql数据库中做初步的格式化
import os
from data.property_collect import law_parse
from data_resource import conn


def wild_animals_parser(file_path):
    dir_path = "C:\\Users\\dhz1216\\Desktop\\washing\\野生动物"
                    ...
                    while line:
                        if len(line.lstrip().split('　')) > 1:
                            ...
                            insert_sql = "insert into law_content (law_id, p_key, p_content, law_class) " \
                                         "value (%s, %s, %s, %s)"
                            try:
                                cursor.execute(insert_sql, (law_id, key_title, value_content, '野生动物'))
                                conn.commit()
...
def wild_animals_filter(file_name):
    invalid_words = ['修订', '修正', '修改']
    if '野生动物' not in file_name:
        return False
...
if __name__ == '__main__':
    ...
    for file in os.listdir(dir_path):
        if wild_animals_filter(file):
            file_path = dir_path + "\\" + file
            wild_animals_parser(file_path)
            # print(file)
            count = count + 1
    print(count)