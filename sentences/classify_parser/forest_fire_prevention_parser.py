# -*- coding : utf-8 -*-
# coding: utf-8
# 森林防火条例相关法律法规的解析---并存入mysql数据库中做初步的格式化
import os
...
def forest_fire_prevention_filter(file_name):
    invalid_words = ['修订', '修正', '修改']
    if '森林防火' not in file_name:
        return False
...
if __name__ == '__main__':
    ...
    for file in os.listdir(dir_path):
        if forest_fire_prevention_filter(file):
            file_path = dir_path + "\\" + file
            forest_fire_prevention_parser(file_path)
            print(file)
            count = count + 1
    print(count)