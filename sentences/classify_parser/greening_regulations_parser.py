# -*- coding : utf-8 -*-
# coding: utf-8
# 绿化条例相关法律法规的解析---并存入mysql数据库中做初步的格式化
import os
from data.property_collect import law_parse
from data_resource import conn


def greening_regulations_parser(file_path):
    dir_path = "C:\\Users\\dhz1216\\Desktop\\washing\\绿化条例"
                ...
                with open(dir_path + "\\" + file_name, "a") as w:
                    ...
                    while line:
                        if len(line.lstrip().split('　')) > 1:
                            ...
                            insert_sql = "insert into law_content (law_id, p_key, p_content, law_class) value (%s, %s, %s, %s)"
                            try:
                                cursor.execute(insert_sql, (law_id, key_title, value_content, '绿化条例'))
                                conn.commit()
...
def greening_regulations_filter(file_name):
    invalid_words = ['修订', '修正', '修改']
    if '绿化' not in file_name:
        return False
...
def green_regulations_to_mysql():
    dir_path = "C:\\Users\\dhz1216\\Desktop\\wenben"
    for file in os.listdir(dir_path):
        if greening_regulations_filter(file):
            file_path = dir_path + "\\" + file
            law_info = law_parse(file_path)
...
if __name__ == '__main__':
    ...
    for file in os.listdir(dir_path):
        if greening_regulations_filter(file):
            file_path = dir_path + "\\" + file
            greening_regulations_parser(file_path)