# -*- coding : utf-8 -*-
# coding: utf-8
# 环境保护条例相关法律法规的解析---并存入mysql数据库中做初步的格式化
import os
from data.property_collect import law_parse
from data_resource import conn


def environment_protection_parser(file_path):
    dir_path = "C:\\Users\\dhz1216\\Desktop\\washing\\环境保护"
                    ...
                    while line:
                        if len(line.lstrip().split('　')) > 1:
                            ...
                            insert_sql = "insert into law_content (law_id, p_key, p_content, law_class) " \
                                         "value (%s, %s, %s, %s)"
                            try:
                                cursor.execute(insert_sql, (law_id, key_title, value_content, '环境保护'))
                                conn.commit()
...
def environment_protection_filter(file_name):
    invalid_words = ['修订', '修正', '修改']
    if '环境保护' not in file_name:
        return False
...
if __name__ == '__main__':
    ...
    for file in os.listdir(dir_path):
        if environment_protection_filter(file):
            # file_path = dir_path + "\\" + file
            # environment_protection_parser(file_path)
            # print(file)
            count = count + 1
    print(count)