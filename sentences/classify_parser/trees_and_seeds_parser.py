# -*- coding : utf-8 -*-
# coding: utf-8
# 林木及种子条例相关法律法规的解析---并存入mysql数据库中做初步的格式化
import os
from data.property_collect import law_parse
from data_resource import conn


def trees_and_seeds_parser(file_path):
    dir_path = "C:\\Users\\dhz1216\\Desktop\\washing\\林木种子"
                    ...
                    while line:
                        if len(line.lstrip().split('　')) > 1:
                            ...
                            insert_sql = "insert into law_content (law_id, p_key, p_content, law_class) " \
                                         "value (%s, %s, %s, %s)"
                            try:
                                cursor.execute(insert_sql, (law_id, key_title, value_content, '林木种子'))
                                conn.commit()
...
def trees_and_seeds_filter(file_name):
    invalid_words = ['修订', '修正', '修改']
    if '林木' not in file_name and '种子' not in file_name:
        return False
...
if __name__ == '__main__':
    ...
    for file in os.listdir(dir_path):
        if trees_and_seeds_filter(file):
            file_path = dir_path + "\\" + file
            trees_and_seeds_parser(file_path)
            print(file)
            count = count + 1
    print(count)